...
var original_jump_velocity = 0.0  # Store the original value
...
func apply_jump_boost():
	...
	if player:
		...
		print("Jump boost applied! Jump velocity:", player.jump_velocity)
		
		# Start timer to remove effect after 10 seconds
		await get_tree().create_timer(10.0).timeout
		
		# Remove boost
		remove_jump_boost(player)

func remove_jump_boost(player):
	if player and effect_active:
		player.jump_velocity =4.5
		effect_active = false
		print("Jump boost expired. Jump velocity restored to:", player.jump_velocity)
